...
public interface FactureRepository extends R2dbcRepository<Facture, UUID> {
    ...
    @Query("SELECT COUNT(*) FROM factures WHERE etat::text = :statut")
    Mono<Long> countByStatut(String statut);

    @Query("SELECT SUM(montant_total) FROM factures WHERE etat::text = :statut")
    ...
    Flux<Facture> findByIdClient(UUID idClient, Pageable pageable);

    Flux<Facture> findByEtat(StatutFacture etat, Pageable pageable);

    @Query("SELECT * FROM factures WHERE date_facturation BETWEEN :startDate AND :endDate")
    Flux<Facture> findByDateFacturationBetweenPaginated(LocalDate startDate, LocalDate endDate, Pageable pageable);

    @Query("SELECT id_client, SUM(montant_total) as total_ventes FROM factures GROUP BY id_client ORDER BY total_ventes DESC LIMIT :limit")
    Flux<com.example.account.modules.facturation.dto.response.ClientChiffreAffaire> findTopClients(int limit);
